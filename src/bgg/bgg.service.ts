...
export class BggService {
  ...
  async searchGames(query: string): Promise<BggGameSearchResult[]> {
    try {
      ...
      const { data } = await lastValueFrom(
        this.httpService.get<string>(url, { params, headers: this.headers }),
      );
      ...
      this.logger.error(
        `Error searching BGG games: ${error.message}`,
        error.stack,
      );
      return [];
    }
  ...
  async getGameDetails(bggId: string): Promise<BggGameDetails> {
    ...
    try {
      ...
      this.logger.error(
        `Error fetching BGG game details for ID ${bggId}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getGameStats(bggId: string): Promise<BggGameStats> {
    try {
      ...
      const weight = stats.averageweight
        ? parseFloat(stats.averageweight.value)
        : 0;
      ...
      this.logger.error(
        `Error fetching BGG game stats for ID ${bggId}: ${error.message}`,
        error.stack,
      );
      return { bggRank: 99999, weight: 0 };
    }
